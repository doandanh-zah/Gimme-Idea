...
import { Heart, Send, Twitter, Copy, Check, Trophy, Clock, DollarSign, Crown, ExternalLink, Wallet } from 'lucide-react';
...
export const Donate = () => {
    ...
    const getSolanaURI = () => {
        const label = encodeURIComponent("Gimme Idea Donation");
        const message = encodeURIComponent("Fueling the revolution! 🚀");
        // URI Schema: solana:<recipient>?amount=<amount>&label=<label>&message=<message>
        return `solana:${walletAddress}?amount=${amount}&label=${label}&message=${message}`;
    };
    ...
    const handleDonate = () => {
        ...
        const uri = getSolanaURI();
        // Open the URI which triggers the wallet
        window.open(uri, '_self');
        toast.success("Opening your wallet...", { icon: '👛' });
    };
                ...
                >
                    <div className="text-left">
                        ...
                        <p className="text-gray-400 text-lg leading-relaxed max-w-md">
                            Your contribution directly supports server costs, coffee, and open-source development on <strong>Mainnet</strong>.
                        </p>
                    </div>
                    ...
                    <div className="bg-[#0F0F0F]/80 backdrop-blur-xl border border-blue-500/20 rounded-3xl p-8 relative overflow-hidden shadow-2xl shadow-blue-900/10 group min-h-[420px] flex flex-col justify-center">
                        ...
                        >
                                ...
                                <div className="grid grid-cols-4 gap-2 mb-4">
                                    {['0.1', '0.5', '1', '2'].map((val) => (
                                        <button
                                            ...
                                            className={`py-3 rounded-xl font-mono font-bold transition-all border ${amount === val
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl font-bold text-white hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2 text-lg border border-white/10 relative overflow-hidden group/btn"
                            >
                                <div className="absolute inset-0 bg-white/20 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-300" />
                                <span className="relative z-10 flex items-center gap-2">
                                    Donate via Wallet <Wallet className="w-5 h-5" />
                                </span>
                            </button>
                            <p className="text-xs text-gray-500 text-center mt-3">
                                Clicking this will open your Solana wallet (Phantom, Solflare) to confirm the transaction on Mainnet.
                            </p>
                        </motion.div>
                    </div>
                            ...
                            {activeTab === 'top' ? (
                                ...
                                >
                                    {topDonators.length > 0 ? (
                                        ...
                                        <div className="p-8 text-center text-gray-500 italic">No top donators yet in Devnet data.</div>
                                    )}
                                </motion.div>
                            ) : (
                                <motion.div
                                ...
                                >
                                    {recentDonations.length > 0 ? (
                                        ...
                                        <div className="p-8 text-center text-gray-500 italic">No recent donations yet.</div>
                                    )}
                                </motion.div>
                            )}
                        </AnimatePresence>
