...
import { Upload, Rocket, X, Image as ImageIcon, Lightbulb, Box, Layers, EyeOff, Eye, Loader2, Trash2 } from 'lucide-react';
...
export const SubmissionModal = () => {
    ...
    const STORAGE_KEY = 'submission_form_draft';
    const IMAGE_STORAGE_KEY = 'submission_form_image';
    ...
    const saveFormDraft = (data: typeof formData, tagsData: string[], imageUrl: string | null) => {
        if (typeof window === 'undefined') return;
        try {
            localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
            localStorage.setItem(IMAGE_STORAGE_KEY, imageUrl || '');
        } catch (error) {
    ...
    // Load form on modal open, save on changes
    useEffect(() => {
        if (isSubmitModalOpen) {
            ...
            if (draft) {
                ...
                // If no draft exists, initialize empty form
                setFormData({
                    title: '', description: '', categories: [], stage: 'Idea', website: '', bounty: '',
                    problem: '', opportunity: '', solution: '', isAnonymous: false
                });
    ...
    useEffect(() => {
        if (isSubmitModalOpen && Object.values(formData).some(v => v !== '' && v !== false && (Array.isArray(v) ? v.length > 0 : true))) {
            saveFormDraft(formData, tags, imagePreview);
        }
    }, [formData, tags, imagePreview, isSubmitModalOpen]);
                        ...
                        <div className="relative bg-[#0D0D12] border border-white/10 rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
                                ...
                                <div className="relative z-10 flex justify-between items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white/5 hover:bg-red-500/20 flex items-center justify-center transition-all text-gray-400 hover:text-red-400 border border-white/10 hover:border-red-500/30 flex-shrink-0"
                                        >
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {categories.map(cat => (
                                                    <button
                                                        ...
                                                        className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${formData.categories.includes(cat)
                                                                ? isProject
                                        ...
                                        <>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div
                                                    ...
                                                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all relative h-48 flex flex-col items-center justify-center overflow-hidden group/upload bg-[#1A1A1A] ${isUploadingImage ? 'border-accent/50 cursor-wait' : 'border-white/10 hover:bg-white/5 hover:border-white/20'
                                                        }`}
                                                >
                            ...
                            <div className="p-6 border-t border-white/10 bg-[#0A0A0F] flex-shrink-0">
                                <button
                                    ...
                                    className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-3 transition-all hover:opacity-90 ${isProject
