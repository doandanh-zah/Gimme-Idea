...
export default function ContactPage() {
  ...
  useEffect(() => {
    const newStars = Array.from({ length: 50 }).map((_, i) => ({
      ...
      size: Math.random() * 2 + 1, duration: \`\${Math.random() * 3 + 2}s\`, opacity: Math.random()
    }));
    setStars(newStars);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch { setIsSubmitting(false); toast.error('Something went wrong. Please try again.'); }
  };
  ...
  return (
    <div className="min-h-screen text-gray-300 pt-20 pb-6 px-4 font-sans relative">
      <div className="fixed inset-0 z-[-1] pointer-events-none overflow-hidden">
        ...
        <div className="stars-container">{stars.map((star) => (<div key={star.id} className="star" style={{ top: star.top, left: star.left, width: \`\${star.size}px\`, height: \`\${star.size}px\`, '--duration': star.duration, '--opacity': star.opacity } as React.CSSProperties} />))}</div>
      </div>
      <div className="max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-1 font-quantico">Get in Touch</h1>
          <p className="text-gray-400 text-sm">We'd love to hear from you. Our team responds within 24-48 hours.</p>
        </motion.div>
