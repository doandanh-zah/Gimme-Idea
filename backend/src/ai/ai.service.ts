...
Respond with JSON only:
{
  ...
  async searchRelatedProjects(
    ...
    try {
      ...
      // Conditional increment is race-safe for the daily max; no separate pre-check.
      const { data: reservedOk, error: reserveError } = await supabase.rpc(
        "increment_search_usage",
        { p_user_id: userId },
      );
      if (reserveError || !reservedOk) {
        const quota = await this.getUserSearchQuota(userId).catch(() => null);
        return {
          success: false,
          error: quota
            ? `Daily search limit reached (${quota.max} searches per day)`
            : "Daily search limit reached. Try again tomorrow.",
          quotaInfo: quota
            ? { remaining: 0, used: quota.used, max: quota.max }
            : undefined,
        };
      }
      ...
      try {
        ...
        await this.refundSearchUsage(userId);
  ...
  private async refundSearchUsage(userId: string): Promise<void> {
    try {
      const supabase = this.supabaseService.getAdminClient();
      const { error: refundError } = await supabase.rpc("refund_search_usage", {
        p_user_id: userId,
      });
      if (refundError) {
        this.logger.error(
      ...
      this.logger.error(`Refund search usage threw for ${userId}:`, refundErr);
    }
  }
  ...
  async clearRelatedProjects(ideaId: string): Promise<{
    ...
    try {
      ...
      // Delete all AI-detected projects for this idea
